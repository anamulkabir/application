...
/**
 * In cryptography, <code>MD5</code> (Message-Digest algorithm 5) is a widely used cryptographic hash function with a 128-bit hash value.
 */
...
public class MD5 {
  ...
  private int ROTATE_LEFT(int x, int n) {return ((x << n) | (x >>> (32-n)));}
...
Rotation is separate from addition to prevent recomputation. */
  ...
  /** MD5 block add operation. Continues an MD5 message-digest
  ...
  public void add(byte[] input, int inputOffset, int inputLen) {
    ...
    if (inputLen >= partLen) {
      ...
      for (i = partLen; i + 63 < inputLen; i += 64) MD5Transform(context.state, input, inputOffset + i);
      index = 0;
    } else {
      i = 0;
    }
  ...
  public byte[] done() {
    ...
    int z;
    for(z=0;z<4;z++) context.state[z] = 0;
    for(z=0;z<64;z++) context.buffer[z] = 0;
    context = null;
